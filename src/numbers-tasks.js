...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const lenght1 = Math.sqrt(x1 * x1 + y1 * y1);
  const lenght2 = Math.sqrt(x2 * x2 + y2 * y2);
  const cosAngle = vectors / (lenght1 * lenght2);
...
function getFibonacciNumber(index) {
  ...
  const fib1 = index - 1;
  const fib2 = index - 2;
  const fib = getFibonacciNumber(fib1) + getFibonacciNumber(fib2);
  return fib;
}
